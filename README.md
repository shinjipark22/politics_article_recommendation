...
* **과거 데이터 수집 (Backfill)**: `catchup=True` 설정을 통해 **2025년 11월 1일**부터 현재까지 누락된 데이터를 자동으로 수집합니다.
...
### 2. 데이터 관리 및 보안
* **데이터 파일 (`data/`)**: 크롤링된 데이터는 개인 로컬 환경에서 관리하며, **GitHub에는 업로드하지 않습니다.** (각 팀원은 11월 1일치부터 데이터를 각자 수집해야 합니다.)
